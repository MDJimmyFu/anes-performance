...
class GitHubDB {
  ...
  /**
   * Load runtime config injected at deploy time from GitHub Secrets.
   * Fetches data/runtime_config.json (generated by CI from GH_PAT_TOKEN secret).
   * If a PAT is found and no token is set in localStorage config, uses it as fallback.
   * Safe to call multiple times; silently ignores errors (file may not exist locally).
   */
  async loadRuntimeConfig() {
    try {
      const resp = await fetch('data/runtime_config.json', { cache: 'no-store' });
      if (!resp.ok) return;
      const cfg = await resp.json();
      if (cfg.gh_pat && !this.token) {
        this.token = cfg.gh_pat;
      }
    } catch {
      // File absent in local dev — ignore
    }
  }
